...
?>
    <div class="row">
        <div class="col-md-12">
			<?php
                echo form_open(base_url() . 'index.php/admin/route/update/' . $row['route_id'], array(
            ...
            ?>
                <div class="panel-body">
                    ...
                    <label class="col-sm-4 control-label" for="route_option"><?php echo translate('route_name');?></label>
                    <div class="col-sm-6">
                        <input type="text" name="name" id="name" 
                          value="<?php echo $row['name'];?>"  placeholder="<?php echo translate('route_name');?>" class="form-control required">
                    </div>
                </div>
                 <div class="form-group btm_border">
                    <label class="col-sm-4 control-label" for="route_status"><?php echo translate('route_latitude');?></label>
                    <div class="col-sm-6">
                        <input type="text" name="latitude" id="latitude" 
                          value="<?php echo $row['latitude'];?>"  placeholder="<?php echo translate('route_latitude');?>" class="form-control required">
                    </div>
                </div>

                <div class="form-group btm_border">
                    <label class="col-sm-4 control-label" for="route_status"><?php echo translate('route_longitude');?></label>
                    <div class="col-sm-6">
                        <input type="text" name="longitude" id="longitude" 
                           value="<?php echo $row['longitude'];?>" placeholder="<?php echo translate('route_longitude');?>" class="form-control required">
                    </div>
                </div>
                    ...
                    <div class="row">
                        <div class="col-md-11">
                            <span class="btn btn-purple btn-labeled fa fa-refresh pro_list_btn pull-right" 
                            	onclick="ajax_set_full('edit','<?php echo translate('edit_route'); ?>','<?php echo translate('successfully_edited!'); ?>','route_edit','<?php echo $row['route_id']; ?>')">
                                	<?php echo translate('reset');?>
                            </span>
                        </div>
                        
                        <div class="col-md-1">
                            <span class="btn btn-success btn-md btn-labeled fa fa-upload pull-right" onclick="form_submit('route_edit','<?php echo translate('successfully_edited!'); ?>')" ><?php echo translate('upload');?></span>
                        </div>
...
?>
    ...
    <script>
        ...
        $(document).ready(function() {
			...
			$('.route_select').each(function(index, element) {
				var a = $(this).val();
		...
		$("#part_num").click(function(){
			...
			htmls =	htmls
				...
				+'					<option value="route"><?php echo translate('route'); ?></option>'
				+'    			</select>'
				+'   		 </div>'
				+'    		<div class="col-sm-12 route" style="display:none;">'
				+'				<select class="demo-cs-multiselect route_select" name="" multiple >'
				...
				+'  		  	</select>'
				+'  		  </div>'
				+'  		  <input type="hidden" name="part_route[]" class="hidd" />'
        ...
        $('body').on('change', '.route_select', function(){	
            var a = $(this).val();
            $(this).closest('.sets').find('.hidd').val(a);
        });
        ...
        $('body').on('change', '.type', function(){	
            ...
            var a = h.val();
            if(a == 'route'){
                h.parent().parent().find('.route').show();
                h.parent().parent().find('.content').hide();
            } else if (a == 'content'){
                h.parent().parent().find('.route').hide();
