...
?>
    <div class="row">
        <div class="col-md-12">
			<?php
                echo form_open(base_url() . 'index.php/admin/vehicle/update/' . $row['vehicle_id'], array(
            ...
            ?>
                <div class="panel-body">
                    ...
                    <label class="col-sm-4 control-label" for="news_title"><?php echo translate('vehicle_name');?></label>
                    <div class="col-sm-6">
                        <input type="text" name="name" id="name" 
                          value="<?php echo $row['name'];?>"  placeholder="<?php echo translate('vehicle_name');?>" class="form-control required">
                    </div>
                </div>
                ...
                <div class="form-group">
                    <label class="col-sm-4 control-label" for="demo-hor-1"><?php echo translate('route');?></label>
                    <div class="col-sm-6">
                        <?php echo $this->crud_model->select_html('route','route_id','name','add','demo-chosen-select','') ?>
                    </div>
                 </div>
                    ...
                    <div class="row">
                        <div class="col-md-11">
                            <span class="btn btn-purple btn-labeled fa fa-refresh pro_list_btn pull-right" 
                            	onclick="ajax_set_full('edit','<?php echo translate('edit_vehicle'); ?>','<?php echo translate('successfully_edited!'); ?>','vehicle_edit','<?php echo $row['vehicle_id']; ?>')">
                                	<?php echo translate('reset');?>
                            </span>
                        </div>
                        
                        <div class="col-md-1">
                            <span class="btn btn-success btn-md btn-labeled fa fa-upload pull-right" onclick="form_submit('vehicle_edit','<?php echo translate('successfully_edited!'); ?>')" ><?php echo translate('upload');?></span>
                        </div>
...
?>
    ...
    <script>
        ...
        $(document).ready(function() {
			...
			$('.vehicle_select').each(function(index, element) {
				var a = $(this).val();
        ...
        $("#vehicle_name").blur(function(){
            var val = $(this).val();
            val = val.split(' ').join('_');
            $("#vehicle_code").val(val);
        });
		...
		$("#part_num").click(function(){
			...
			htmls =	htmls
				...
				+'					<option value="vehicle"><?php echo translate('vehicle'); ?></option>'
				+'    			</select>'
				+'   		 </div>'
				+'    		<div class="col-sm-12 vehicle" style="display:none;">'
				+'				<select class="demo-cs-multiselect vehicle_select" name="" multiple >'
				...
				+'  		  	</select>'
				+'  		  </div>'
				+'  		  <input type="hidden" name="part_vehicle[]" class="hidd" />'
        ...
        $('body').on('change', '.vehicle_select', function(){	
            var a = $(this).val();
            $(this).closest('.sets').find('.hidd').val(a);
        });
        ...
        $('body').on('change', '.type', function(){	
            ...
            var a = h.val();
            if(a == 'vehicle'){
                h.parent().parent().find('.vehicle').show();
                h.parent().parent().find('.content').hide();
            } else if (a == 'content'){
                h.parent().parent().find('.vehicle').hide();
