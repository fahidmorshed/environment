...
?>
    <div class="row">
        <div class="col-md-12">
			<?php
                echo form_open(base_url() . 'index.php/admin/funding/update/' . $row['funding_id'], array(
            ...
            ?>
                ...
                <div class="form-group">
                 ...
                 <div class="form-group btm_border">
                    <label class="col-sm-4 control-label" for="funding_title"><?php echo translate('funding_title');?></label>
                    <div class="col-sm-6">
                        <input type="text" name="title" id="title" 
                            placeholder="<?php echo translate('funding_title');?>" class="form-control required" value="<?php echo $row['title']; ?>">
                    </div>
                </div>
                
                <div class="form-group btm_border">
                    <label class="col-sm-4 control-label" for="funding_title"><?php echo translate('funding_description');?></label>
                    <div class="col-sm-6">
                        <input type="text" name="description" id="description" 
                            placeholder="<?php echo translate('funding_description');?>" class="form-control required" value="<?php echo $row['description']; ?>">
                    </div>
                </div>
                    ...
                    <div class="form-group btm_border">
                    <label class="col-sm-4 control-label" for="funding_status"><?php echo translate('funding_remark');?></label>
                    <div class="col-sm-6">
                        <input type="text" name="remark" id="remark" 
                            placeholder="<?php echo translate('funding_remark');?>" class="form-control required" value="<?php echo $row['remark']; ?>">
                    </div>
                </div>
                    ...
                    <div class="row">
                        <div class="col-md-11">
                            <span class="btn btn-purple btn-labeled fa fa-refresh pro_list_btn pull-right" 
                            	onclick="ajax_set_full('edit','<?php echo translate('edit_funding'); ?>','<?php echo translate('successfully_edited!'); ?>','funding_edit','<?php echo $row['funding_id']; ?>')">
                                	<?php echo translate('reset');?>
                            </span>
                        </div>
                        
                        <div class="col-md-1">
                            <span class="btn btn-success btn-md btn-labeled fa fa-upload pull-right" onclick="form_submit('funding_edit','<?php echo translate('successfully_edited!'); ?>')" ><?php echo translate('upload');?></span>
                        </div>
...
?>
    ...
    <script>
        ...
        $(document).ready(function() {
			...
			$('.funding_select').each(function(index, element) {
				var a = $(this).val();
        ...
        $("#funding_name").blur(function(){
            var val = $(this).val();
            val = val.split(' ').join('_');
            $("#funding_code").val(val);
        });
		...
		$("#part_num").click(function(){
			...
			htmls =	htmls
				...
				+'					<option value="funding"><?php echo translate('funding'); ?></option>'
				+'    			</select>'
				+'   		 </div>'
				+'    		<div class="col-sm-12 funding" style="display:none;">'
				+'				<select class="demo-cs-multiselect funding_select" name="" multiple >'
				...
				+'  		  	</select>'
				+'  		  </div>'
				+'  		  <input type="hidden" name="part_funding[]" class="hidd" />'
        ...
        $('body').on('change', '.funding_select', function(){	
            var a = $(this).val();
            $(this).closest('.sets').find('.hidd').val(a);
        });
        ...
        $('body').on('change', '.type', function(){	
            ...
            var a = h.val();
            if(a == 'funding'){
                h.parent().parent().find('.funding').show();
                h.parent().parent().find('.content').hide();
            } else if (a == 'content'){
                h.parent().parent().find('.funding').hide();
