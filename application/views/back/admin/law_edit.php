...
?>
    <div class="row">
        <div class="col-md-12">
			<?php
                echo form_open(base_url() . 'index.php/admin/law/update/' . $row['law_id'], array(
            ...
            ?>
                ...
                <div class="form-group abstract">
                  <label class="col-sm-4 control-label"><?php echo translate('law_name');?></label>
                  <div class="col-sm-6">
                    <textarea class="summernotes"  data-height="200" data-name="law_name" ><?php echo $row['law_name'];?></textarea>
                  </div>
               </div>

                <div class="form-group abstract">
                  <label class="col-sm-4 control-label"><?php echo translate('Description');?></label>
                  <div class="col-sm-6">
                    ...
                    <div class="row">
                        <div class="col-md-11">
                            <span class="btn btn-purple btn-labeled fa fa-refresh pro_list_btn pull-right" 
                            	onclick="ajax_set_full('edit','<?php echo translate('edit_law'); ?>','<?php echo translate('successfully_edited!'); ?>','law_edit','<?php echo $row['law_id']; ?>')">
                                	<?php echo translate('reset');?>
                            </span>
                        </div>
                        
                        <div class="col-md-1">
                            <span class="btn btn-success btn-md btn-labeled fa fa-upload pull-right" onclick="form_submit('law_edit','<?php echo translate('successfully_edited!'); ?>')" ><?php echo translate('upload');?></span>
                        </div>
...
?>
    ...
    <script>
        ...
        $(document).ready(function() {
			...
			$('.law_select').each(function(index, element) {
				var a = $(this).val();
		...
		$("#part_num").click(function(){
			...
			htmls =	htmls
				...
				+'					<option value="law"><?php echo translate('law'); ?></option>'
				+'    			</select>'
				+'   		 </div>'
				+'    		<div class="col-sm-12 law" style="display:none;">'
				+'				<select class="demo-cs-multiselect law_select" name="" multiple >'
				...
				+'  		  	</select>'
				+'  		  </div>'
				+'  		  <input type="hidden" name="part_law[]" class="hidd" />'
        ...
        $('body').on('change', '.law_select', function(){	
            var a = $(this).val();
            $(this).closest('.sets').find('.hidd').val(a);
        });
        ...
        $('body').on('change', '.type', function(){	
            ...
            var a = h.val();
            if(a == 'law'){
                h.parent().parent().find('.law').show();
                h.parent().parent().find('.content').hide();
            } else if (a == 'content'){
                h.parent().parent().find('.law').hide();
