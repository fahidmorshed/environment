<?php
    foreach($noisy_point_data as $row){
?>
    <div class="row">
        <div class="col-md-12">
			<?php
                echo form_open(base_url() . 'index.php/admin/noisy_point/update/' . $row['noisy_point_id'], array(
            ...
            ?>
                <div class="panel-body">
                 ...
                 <div class="form-group btm_border">
                    <label class="col-sm-4 control-label" for="noisy_point_name"><?php echo translate('noisy_point_name');?></label>
                    <div class="col-sm-6">
                        <input type="text" name="name" id="name" 
                         value="<?php echo $row['name']; ?>"   placeholder="<?php echo translate('noisy_point_name');?>" class="form-control required">
                    </div>
                </div>
               ...
               <div class="form-group">
                 ...
                 <div class="form-group">
                    <label class="col-sm-4 control-label" for="demo-hor-1"><?php echo translate('noisy_point_type');?></label>
                    <div class="col-sm-6">
                        <?php echo $this->crud_model->select_html('noisy_point_type','noisy_point_type_id','type','add','demo-chosen-select','') ?>
                    </div>
                 </div>

                <div class="form-group ">
                    <label class="col-sm-4 control-label"><?php echo translate('noisy_point_reason');?></label>
                ...
                <div class="form-group ">
                    <label class="col-sm-4 control-label"><?php echo translate('noisy_point_action');?></label>
                   <div class="col-sm-6 abstract">
                    ...
                    <div class="row">
                        <div class="col-md-11">
                            <span class="btn btn-purple btn-labeled fa fa-refresh pro_list_btn pull-right" 
                            	onclick="ajax_set_full('edit','<?php echo translate('edit_noisy_point'); ?>','<?php echo translate('successfully_edited!'); ?>','noisy_point_edit','<?php echo $row['noisy_point_id']; ?>')">
                                	<?php echo translate('reset');?>
                            </span>
                        </div>
                        
                        <div class="col-md-1">
                            <span class="btn btn-success btn-md btn-labeled fa fa-upload pull-right" onclick="form_submit('noisy_point_edit','<?php echo translate('successfully_edited!'); ?>')" ><?php echo translate('upload');?></span>
                        </div>
...
?>
    ...
    <script>
        ...
        $(document).ready(function() {
			...
			$('.noisy_point_select').each(function(index, element) {
				var a = $(this).val();
        ...
        $("#noisy_point_name").blur(function(){
            var val = $(this).val();
            val = val.split(' ').join('_');
            $("#noisy_point_code").val(val);
        });
		...
		$("#part_num").click(function(){
			...
			htmls =	htmls
				...
				+'					<option value="noisy_point"><?php echo translate('noisy_point'); ?></option>'
				+'    			</select>'
				+'   		 </div>'
				+'    		<div class="col-sm-12 noisy_point" style="display:none;">'
				+'				<select class="demo-cs-multiselect noisy_point_select" name="" multiple >'
				...
				+'  		  	</select>'
				+'  		  </div>'
				+'  		  <input type="hidden" name="part_noisy_point[]" class="hidd" />'
        ...
        $('body').on('change', '.noisy_point_select', function(){	
            var a = $(this).val();
            $(this).closest('.sets').find('.hidd').val(a);
        });
        ...
        $('body').on('change', '.type', function(){	
            ...
            var a = h.val();
            if(a == 'noisy_point'){
                h.parent().parent().find('.noisy_point').show();
                h.parent().parent().find('.content').hide();
            } else if (a == 'content'){
                h.parent().parent().find('.noisy_point').hide();
