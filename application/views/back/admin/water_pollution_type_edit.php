<?php
    foreach($water_pollution_type_data as $row){
?>
    <div class="row">
        <div class="col-md-12">
			<?php
                echo form_open(base_url() . 'index.php/admin/water_pollution_type/update/' . $row['water_pollution_type_id'], array(
                    'class' => 'form-horizontal',
                    'method' => 'post',
                    'id' => 'water_pollution_type_edit',
            ...
            ?>
                <div class="panel-body">
                 ...
                 <div class="form-group btm_border">
                    <label class="col-sm-4 control-label" for="water_pollution_type_option"><?php echo translate('water_pollution_type_type');?></label>
                    <div class="col-sm-6">
                        <input type="text" name="type" id="type" 
                            placeholder="<?php echo translate('water_pollution_type');?>" class="form-control required" value="<?php echo $row['type']; ?>">
                    </div>
                </div>
                    ...
                    <div class="row">
                        <div class="col-md-11">
                            <span class="btn btn-purple btn-labeled fa fa-refresh pro_list_btn pull-right" 
                            	onclick="ajax_set_full('edit','<?php echo translate('edit_water_pollution_type'); ?>','<?php echo translate('successfully_edited!'); ?>','water_pollution_type_edit','<?php echo $row['water_pollution_type_id']; ?>')">
                                	<?php echo translate('reset');?>
                            </span>
                        </div>
                        
                        <div class="col-md-1">
                            <span class="btn btn-success btn-md btn-labeled fa fa-upload pull-right" onclick="form_submit('water_pollution_type_edit','<?php echo translate('successfully_edited!'); ?>')" ><?php echo translate('upload');?></span>
                        </div>
...
?>
    ...
    <script>
        ...
        $(document).ready(function() {
			...
			$('.water_pollution_type_select').each(function(index, element) {
				var a = $(this).val();
        ...
        $("#water_pollution_type_name").blur(function(){
            var val = $(this).val();
            val = val.split(' ').join('_');
            $("#water_pollution_type_code").val(val);
        });
		...
		$("#part_num").click(function(){
			...
			htmls =	htmls
				...
				+'					<option value="water_pollution_type"><?php echo translate('water_pollution_type'); ?></option>'
				+'    			</select>'
				+'   		 </div>'
				+'    		<div class="col-sm-12 water_pollution_type" style="display:none;">'
				+'				<select class="demo-cs-multiselect water_pollution_type_select" name="" multiple >'
				...
				+'  		  	</select>'
				+'  		  </div>'
				+'  		  <input type="hidden" name="part_water_pollution_type[]" class="hidd" />'
        ...
        $('body').on('change', '.water_pollution_type_select', function(){	
            var a = $(this).val();
            $(this).closest('.sets').find('.hidd').val(a);
        });
        ...
        $('body').on('change', '.type', function(){	
            var h = $(this);
            var a = h.val();
            if(a == 'water_pollution_type'){
                h.parent().parent().find('.water_pollution_type').show();
                h.parent().parent().find('.content').hide();
            } else if (a == 'content'){
                h.parent().parent().find('.water_pollution_type').hide();
