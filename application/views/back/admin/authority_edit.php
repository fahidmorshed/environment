<?php
    foreach($authority_data as $row){
?>
    <div class="row" >
        <div class="col-md-12">
			<?php
                echo form_open(base_url() . 'index.php/admin/authority/update/' . $row['authority_id'], array(
            ...
            ?>
                <div class="panel-body">
                    <div class="form-group abstract">
                        <label class="col-sm-4 control-label"><?php echo translate('authority_question');?></label>
                        <div class="col-sm-6">
                            <textarea class="summernotes"  data-height="200" data-name="question" ><?php echo $row['question'];?></textarea>
                        </div>
                   </div>


                 <div class="form-group btm_border">
                    <label class="col-sm-4 control-label" for="authority_option"><?php echo translate('authority_options');?></label>
                    <div class="col-sm-6">
                        <input type="text" name="options" id="options" 
                            placeholder="<?php echo translate('authority_options');?>" class="form-control required" value="<?php echo $row['options']; ?>">
                    </div>
                </div>
                 <div class="form-group btm_border">
                    <label class="col-sm-4 control-label" for="authority_status"><?php echo translate('authority_status');?></label>
                    <div class="col-sm-6">
                        <input type="text" name="status" id="status" 
                            placeholder="<?php echo translate('authority_status');?>" class="form-control required" value="<?php echo $row['status']; ?>">
                    </div>
                </div>
                    ...
                    <div class="row">
                        <div class="col-md-11">
                            <span class="btn btn-purple btn-labeled fa fa-refresh pro_list_btn pull-right" 
                            	onclick="ajax_set_full('edit','<?php echo translate('edit_authority'); ?>','<?php echo translate('successfully_edited!'); ?>','authority_edit','<?php echo $row['authority_id']; ?>')">
                                	<?php echo translate('reset');?>
                            </span>
                        </div>
                        
                        <div class="col-md-1">
                            <span class="btn btn-success btn-md btn-labeled fa fa-upload pull-right" onclick="form_submit('authority_edit','<?php echo translate('successfully_edited!'); ?>')" ><?php echo translate('upload');?></span>
                        </div>
...
?>
    ...
    <script>
        ...
        $(document).ready(function() {
			...
			$('.authority_select').each(function(index, element) {
				var a = $(this).val();
        ...
        $("#authority_name").blur(function(){
            var val = $(this).val();
            val = val.split(' ').join('_');
            $("#authority_code").val(val);
        });
		...
		$("#part_num").click(function(){
			...
			htmls =	htmls
				...
				+'					<option value="authority"><?php echo translate('authority'); ?></option>'
				+'    			</select>'
				+'   		 </div>'
				+'    		<div class="col-sm-12 authority" style="display:none;">'
				+'				<select class="demo-cs-multiselect authority_select" name="" multiple >'
				...
				+'  		  	</select>'
				+'  		  </div>'
				+'  		  <input type="hidden" name="part_authority[]" class="hidd" />'
        ...
        $('body').on('change', '.authority_select', function(){	
            var a = $(this).val();
            $(this).closest('.sets').find('.hidd').val(a);
        });
        ...
        $('body').on('change', '.type', function(){	
            ...
            var a = h.val();
            if(a == 'authority'){
                h.parent().parent().find('.authority').show();
                h.parent().parent().find('.content').hide();
            } else if (a == 'content'){
                h.parent().parent().find('.authority').hide();
