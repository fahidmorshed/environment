        ...
        <div class="col-md-12">
            ...
            ?>
                <div class="panel-body">
                    ...
                    <label class="col-sm-4 control-label" for="area_name"><?php echo translate('area_name');?></label>
                    <div class="col-sm-6">
                        <input type="text" name="name" id="name" 
                           value="<?php echo $row['name']?>" placeholder="<?php echo translate('area_name');?>" class="form-control required" >
                    </div>
                </div>
                
                <div class="form-group btm_border">
                    <label class="col-sm-4 control-label" for="area_people"><?php echo translate('area_people');?></label>
                    <div class="col-sm-6">
                        <input type="text" name="people" id="people" 
                          value="<?php echo $row['people']?>"   placeholder="<?php echo translate('area_people');?>" class="form-control required">
                    </div>
                </div>

               <div class="form-group btm_border">
                    <label class="col-sm-4 control-label" for="area_length"><?php echo translate('area_length');?></label>
                    <div class="col-sm-6">
                        <input type="text" name="length" id="length" 
                          value="<?php echo $row['length']?>"   placeholder="<?php echo translate('area_length');?>" class="form-control required">
                    </div>
                </div>

                 <div class="form-group btm_border">
                    <label class="col-sm-4 control-label" for="area_block"><?php echo translate('area_block');?></label>
                    <div class="col-sm-6">
                        <input type="text" name="block" id="block" 
                          value="<?php echo $row['block']?>"   placeholder="<?php echo translate('area_block');?>" class="form-control required">
                    </div>
                </div>

                <div class="form-group">
                  <label class="col-sm-4 control-label" for="demo-hor-1"><?php echo translate('union');?></label>
                  <div class="col-sm-6">
                      <?php echo $this->crud_model->select_html('union','union_id','name','edit','demo-chosen-select','') ?>
                  </div>
                </div>

                <div class="form-group">
                  <label class="col-sm-4 control-label" for="demo-hor-1"><?php echo translate('authority');?></label>
                  <div class="col-sm-6">
                      <?php echo $this->crud_model->select_html('admin','admin_id','name','edit','demo-chosen-select','') ?>
                  </div>
                </div>
                    ...
                    <div class="row">
                        <div class="col-md-11">
                            <span class="btn btn-purple btn-labeled fa fa-refresh pro_list_btn pull-right" 
                            	onclick="ajax_set_full('edit','<?php echo translate('edit_area'); ?>','<?php echo translate('successfully_edited!'); ?>','area_edit','<?php echo $row['area_id']; ?>')">
                                	<?php echo translate('reset');?>
                            </span>
                        </div>
                        
                        <div class="col-md-1">
                            <span class="btn btn-success btn-md btn-labeled fa fa-upload pull-right" onclick="form_submit('area_edit','<?php echo translate('successfully_edited!'); ?>')" ><?php echo translate('upload');?></span>
                        </div>
...
?>
    ...
    <script>
        ...
        $(document).ready(function() {
			...
			$('.area_select').each(function(index, element) {
				var a = $(this).val();
		...
		$("#part_num").click(function(){
			...
			htmls =	htmls
				...
				+'					<option value="area"><?php echo translate('area'); ?></option>'
				+'    			</select>'
				+'   		 </div>'
				+'    		<div class="col-sm-12 area" style="display:none;">'
				+'				<select class="demo-cs-multiselect area_select" name="" multiple >'
				...
				+'  		  	</select>'
				+'  		  </div>'
				+'  		  <input type="hidden" name="part_area[]" class="hidd" />'
        ...
        $('body').on('change', '.type', function(){	
            ...
            var a = h.val();
            if(a == 'area'){
                h.parent().parent().find('.area').show();
                h.parent().parent().find('.content').hide();
            } else if (a == 'content'){
                h.parent().parent().find('.area').hide();
