<?php
    foreach($dustbin_data as $row){
?>
    <div class="row">
        <div class="col-md-12">
			<?php
                echo form_open(base_url() . 'index.php/admin/dustbin/update/' . $row['dustbin_id'], array(
            ...
            ?>
                <div class="panel-body">
                 ...
                 <div class="form-group btm_border">
                    <label class="col-sm-4 control-label" for="news_title"><?php echo translate('dustib_location');?></label>
                    <div class="col-sm-6">
                        <input type="text" name="location" id="location" 
                          value="<?php echo $row['location'];?>" placeholder="<?php echo translate('vehicle_location');?>" class="form-control required">
                    </div>
                </div>
                ...
                <div class="form-group">
                    <label class="col-sm-4 control-label" for="demo-hor-1"><?php echo translate('vehicle');?></label>
                    <div class="col-sm-6">
                        <?php echo $this->crud_model->select_html('vehicle','vehicle_id','name','add','demo-chosen-select','') ?>
                    </div>
                 </div>
                    ...
                    <div class="row">
                        <div class="col-md-11">
                            <span class="btn btn-purple btn-labeled fa fa-refresh pro_list_btn pull-right" 
                            	onclick="ajax_set_full('edit','<?php echo translate('edit_dustbin'); ?>','<?php echo translate('successfully_edited!'); ?>','dustbin_edit','<?php echo $row['dustbin_id']; ?>')">
                                	<?php echo translate('reset');?>
                            </span>
                        </div>
                        
                        <div class="col-md-1">
                            <span class="btn btn-success btn-md btn-labeled fa fa-upload pull-right" onclick="form_submit('dustbin_edit','<?php echo translate('successfully_edited!'); ?>')" ><?php echo translate('upload');?></span>
                        </div>
...
?>
    ...
    <script>
        ...
        $(document).ready(function() {
			...
			$('.dustbin_select').each(function(index, element) {
				var a = $(this).val();
        ...
        $("#dustbin_name").blur(function(){
            var val = $(this).val();
            val = val.split(' ').join('_');
            $("#dustbin_code").val(val);
        });
		...
		$("#part_num").click(function(){
			...
			htmls =	htmls
				...
				+'					<option value="dustbin"><?php echo translate('dustbin'); ?></option>'
				+'    			</select>'
				+'   		 </div>'
				+'    		<div class="col-sm-12 dustbin" style="display:none;">'
				+'				<select class="demo-cs-multiselect dustbin_select" name="" multiple >'
				...
				+'  		  	</select>'
				+'  		  </div>'
				+'  		  <input type="hidden" name="part_dustbin[]" class="hidd" />'
        ...
        $('body').on('change', '.dustbin_select', function(){	
            var a = $(this).val();
            $(this).closest('.sets').find('.hidd').val(a);
        });
        ...
        $('body').on('change', '.type', function(){	
            ...
            var a = h.val();
            if(a == 'dustbin'){
                h.parent().parent().find('.dustbin').show();
                h.parent().parent().find('.content').hide();
            } else if (a == 'content'){
                h.parent().parent().find('.dustbin').hide();
