...
?>
    <div class="row">
        <div class="col-md-12">
			<?php
                echo form_open(base_url() . 'index.php/admin/review/update/' . $row['review_id'], array(
            ...
            ?>
                <div class="panel-body">
                    <div class="form-group abstract">
                        <label class="col-sm-4 control-label"><?php echo translate('review_question');?></label>
                        <div class="col-sm-6">
                 ...
                 <div class="form-group btm_border">
                    <label class="col-sm-4 control-label" for="review_option"><?php echo translate('review_options');?></label>
                    <div class="col-sm-6">
                        <input type="text" name="options" id="options" 
                            placeholder="<?php echo translate('review_options');?>" class="form-control required" value="<?php echo $row['options']; ?>">
                    </div>
                </div>
                 <div class="form-group btm_border">
                    <label class="col-sm-4 control-label" for="review_status"><?php echo translate('review_status');?></label>
                    <div class="col-sm-6">
                        <input type="text" name="status" id="status" 
                            placeholder="<?php echo translate('review_status');?>" class="form-control required" value="<?php echo $row['status']; ?>">
                    </div>
                </div>
                    ...
                    <div class="row">
                        <div class="col-md-11">
                            <span class="btn btn-purple btn-labeled fa fa-refresh pro_list_btn pull-right" 
                            	onclick="ajax_set_full('edit','<?php echo translate('edit_review'); ?>','<?php echo translate('successfully_edited!'); ?>','review_edit','<?php echo $row['review_id']; ?>')">
                                	<?php echo translate('reset');?>
                            </span>
                        </div>
                        
                        <div class="col-md-1">
                            <span class="btn btn-success btn-md btn-labeled fa fa-upload pull-right" onclick="form_submit('review_edit','<?php echo translate('successfully_edited!'); ?>')" ><?php echo translate('upload');?></span>
                        </div>
...
?>
    ...
    <script>
        ...
        $(document).ready(function() {
			...
			$('.review_select').each(function(index, element) {
				var a = $(this).val();
		...
		$("#part_num").click(function(){
			...
			htmls =	htmls
				...
				+'					<option value="review"><?php echo translate('review'); ?></option>'
				+'    			</select>'
				+'   		 </div>'
				+'    		<div class="col-sm-12 review" style="display:none;">'
				+'				<select class="demo-cs-multiselect review_select" name="" multiple >'
				...
				+'  		  	</select>'
				+'  		  </div>'
				+'  		  <input type="hidden" name="part_review[]" class="hidd" />'
        ...
        $('body').on('change', '.review_select', function(){	
            var a = $(this).val();
            $(this).closest('.sets').find('.hidd').val(a);
        });
        ...
        $('body').on('change', '.type', function(){	
            ...
            var a = h.val();
            if(a == 'review'){
                h.parent().parent().find('.review').show();
                h.parent().parent().find('.content').hide();
            } else if (a == 'content'){
                h.parent().parent().find('.review').hide();
