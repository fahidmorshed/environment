...
?>
    <div class="row">
        <div class="col-md-12">
			<?php
                echo form_open(base_url() . 'index.php/admin/public1/update/' . $row['public_id'], array(
            ...
            ?>
                <div class="panel-body">
                    <div class="form-group abstract">
                        <label class="col-sm-4 control-label"><?php echo translate('public_question');?></label>
                        <div class="col-sm-6">
                 ...
                 <div class="form-group btm_border">
                    <label class="col-sm-4 control-label" for="public_option"><?php echo translate('public_options');?></label>
                    <div class="col-sm-6">
                        <input type="text" name="options" id="options" 
                            placeholder="<?php echo translate('public_options');?>" class="form-control required" value="<?php echo $row['options']; ?>">
                    </div>
                </div>
                 <div class="form-group btm_border">
                    <label class="col-sm-4 control-label" for="public_status"><?php echo translate('public_status');?></label>
                    <div class="col-sm-6">
                        <input type="text" name="status" id="status" 
                            placeholder="<?php echo translate('public_status');?>" class="form-control required" value="<?php echo $row['status']; ?>">
                    </div>
                </div>
                    ...
                    <div class="row">
                        <div class="col-md-11">
                            <span class="btn btn-purple btn-labeled fa fa-refresh pro_list_btn pull-right" 
                            	onclick="ajax_set_full('edit','<?php echo translate('edit_public'); ?>','<?php echo translate('successfully_edited!'); ?>','public_edit','<?php echo $row['public_id']; ?>')">
                                	<?php echo translate('reset');?>
                            </span>
                        </div>
                        
                        <div class="col-md-1">
                            <span class="btn btn-success btn-md btn-labeled fa fa-upload pull-right" onclick="form_submit('public_edit','<?php echo translate('successfully_edited!'); ?>')" ><?php echo translate('upload');?></span>
                        </div>
...
?>
    ...
    <script>
        ...
        $(document).ready(function() {
			...
			$('.public_select').each(function(index, element) {
				var a = $(this).val();
		...
		$("#part_num").click(function(){
			...
			htmls =	htmls
				...
				+'					<option value="public"><?php echo translate('public'); ?></option>'
				+'    			</select>'
				+'   		 </div>'
				+'    		<div class="col-sm-12 public" style="display:none;">'
				+'				<select class="demo-cs-multiselect public_select" name="" multiple >'
				...
				+'  		  	</select>'
				+'  		  </div>'
				+'  		  <input type="hidden" name="part_public[]" class="hidd" />'
        ...
        $('body').on('change', '.public_select', function(){	
            var a = $(this).val();
            $(this).closest('.sets').find('.hidd').val(a);
        });
        ...
        $('body').on('change', '.type', function(){	
            ...
            var a = h.val();
            if(a == 'public'){
                h.parent().parent().find('.public').show();
                h.parent().parent().find('.content').hide();
            } else if (a == 'content'){
                h.parent().parent().find('.public').hide();
