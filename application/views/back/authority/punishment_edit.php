...
?>
    <div class="row">
        <div class="col-md-12">
			<?php
                echo form_open(base_url() . 'index.php/admin/punishment/update/' . $row['punishment_id'], array(
            ...
            ?>
                <div class="panel-body">
                    <div class="form-group abstract">
                        <label class="col-sm-4 control-label"><?php echo translate('punishment_question');?></label>
                        <div class="col-sm-6">
                 ...
                 <div class="form-group btm_border">
                    <label class="col-sm-4 control-label" for="punishment_option"><?php echo translate('punishment_options');?></label>
                    <div class="col-sm-6">
                        <input type="text" name="options" id="options" 
                            placeholder="<?php echo translate('punishment_options');?>" class="form-control required" value="<?php echo $row['options']; ?>">
                    </div>
                </div>
                 <div class="form-group btm_border">
                    <label class="col-sm-4 control-label" for="punishment_status"><?php echo translate('punishment_status');?></label>
                    <div class="col-sm-6">
                        <input type="text" name="status" id="status" 
                            placeholder="<?php echo translate('punishment_status');?>" class="form-control required" value="<?php echo $row['status']; ?>">
                    </div>
                </div>
                    ...
                    <div class="row">
                        <div class="col-md-11">
                            <span class="btn btn-purple btn-labeled fa fa-refresh pro_list_btn pull-right" 
                            	onclick="ajax_set_full('edit','<?php echo translate('edit_punishment'); ?>','<?php echo translate('successfully_edited!'); ?>','punishment_edit','<?php echo $row['punishment_id']; ?>')">
                                	<?php echo translate('reset');?>
                            </span>
                        </div>
                        
                        <div class="col-md-1">
                            <span class="btn btn-success btn-md btn-labeled fa fa-upload pull-right" onclick="form_submit('punishment_edit','<?php echo translate('successfully_edited!'); ?>')" ><?php echo translate('upload');?></span>
                        </div>
...
?>
    ...
    <script>
        ...
        $(document).ready(function() {
			...
			$('.punishment_select').each(function(index, element) {
				var a = $(this).val();
        ...
        $("#punishment_name").blur(function(){
            var val = $(this).val();
            val = val.split(' ').join('_');
            $("#punishment_code").val(val);
        });
		...
		$("#part_num").click(function(){
			...
			htmls =	htmls
				...
				+'					<option value="punishment"><?php echo translate('punishment'); ?></option>'
				+'    			</select>'
				+'   		 </div>'
				+'    		<div class="col-sm-12 punishment" style="display:none;">'
				+'				<select class="demo-cs-multiselect punishment_select" name="" multiple >'
				...
				+'  		  	</select>'
				+'  		  </div>'
				+'  		  <input type="hidden" name="part_punishment[]" class="hidd" />'
        ...
        $('body').on('change', '.punishment_select', function(){	
            var a = $(this).val();
            $(this).closest('.sets').find('.hidd').val(a);
        });
        ...
        $('body').on('change', '.type', function(){	
            ...
            var a = h.val();
            if(a == 'punishment'){
                h.parent().parent().find('.punishment').show();
                h.parent().parent().find('.content').hide();
            } else if (a == 'content'){
                h.parent().parent().find('.punishment').hide();
