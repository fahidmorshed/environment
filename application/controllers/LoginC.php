...
class LoginC extends CI_Controller{
    ...
    function do_login()
    {
        ...
        $query      =   $this->db->get_where('public' , array('email' => $email , 'password' => $password));
        if ($query->num_rows() > 0)
        {
            $row = $query->row();
            $this->session->set_userdata('user_name', $row->name);

            $this->session->set_userdata('user_id', $row->public_id);
            redirect(base_url() . 'index.php/homeC' , 'refresh');
    ...
    function register($param1 = '')
    {
       //echo 'dsdsdsd';
        ...
        if ($param1 == 'create') 
        {
            ...
            $check_mail = $this->db->get_where('public', array('email' => $data['email']));
            if($check_mail->num_rows() > 0){
                ...
                $query      =   $this->db->get_where('public' , array('email' => $data['email'] , 'password' => $data['password']));
                if ($query->num_rows() > 0)
                {
                    $row = $query->row();
                    $this->session->set_userdata('user_name', $row->name);
