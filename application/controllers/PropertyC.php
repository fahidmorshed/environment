...
class PropertyC extends CI_Controller{
    ...
    public function viewsingleproperty($param=""){
    	$innitiative_type = $param;
   		$sql = "SELECT * FROM `property` WHERE property_id=?";  
        $query = $this->user_model->get_innitiatives() ;
        if ($query->num_rows() > 0)
        {
            //$update_data = array('view_count' => "1");
            $fahid_row = $query->row();
            $view_count = $fahid_row->view_count;
            $view_count = $view_count + 1;
            $update_data = array('view_count' => $view_count);
            ...
            foreach ($query->result() as $row)
            {
                ...
                $avg1 = 0 ;
                $avg2 = 0 ;
            	$sql_1 = "SELECT * FROM `record` WHERE property_id=?" ;
                $query_1 = $this->db->query($sql_1,array($property_id)) ;
                if($query_1->num_rows()>0){
            	...
            	$sql_2 = "SELECT * FROM `review` WHERE property_id=?" ;
                $query_2 = $this->db->query($sql_2,array($property_id)) ;
                $review = $query_2 ;
               // echo $review ;
                $avg1 = 0 ;
                $avg2 = 0 ;
                if($query_2->num_rows()>0){
                    foreach ($query_2->result() as $row1) {
                        $avg1++ ;
                        $avg2 += $row1->rating ;
 ...
 //     	print_r($review) ;
        ...
        $this->db->where('property_id', $property_id);
        if(!$this->db->update('property', $temp_data)){
            echo "Something went wrong.";
        }

        $page_data['page_name']     = "showsingleproperty";
        $page_data['page_title']    = translate('Property Details');
        $this->load->view('front/index', $page_data);
    }
